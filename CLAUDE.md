...
**Next (roadmap, not yet built):** `rai-release-notes` (Phase 7 — net-new; aggregate both repos' merged PRs, anchor to the PRD, emit a Support handoff). Demoted to docs/infra, *not* skills: cross-repo PR review (community `review` + the rubric in `rai-ready-to-code`; a thin override only if cross-repo review is needed), the contract *format* (a one-time ADR), regression/CI, and Jira intake. (The **ReadyToCode gate** was demoted here originally — "community `triage` + a checklist doc" — but dogfooding showed a checklist a builder reads cannot *run* the adversarial review the gate needs, so it is now the `rai-ready-to-code` skill.)
