...
**The rulebook + the check (the shared "done").** "What complete looks like" for each artifact — the PRD, the shaped seams / `architect-questions.md`, the `contract/`, `feature.json` — no longer lives as prose re-described inside each skill (which the suite audit found drifts). It lives **once**, as a versioned **rulebook**, in the lifecycle engine (`RAI.ADLC`). Each pipeline skill now **cites the rulebook and runs the check** (`rai-adlc check <feature>` → **done / incomplete / absent** + the missing pieces) at its handoff, so the bar can't disagree across skills and the agent self-corrects before handing off — a human never types "validate". A skill *coaches how to produce* the artifact; the rulebook *defines when it's done* (RAI.ADLC ADR-0021). Where each product's artifacts live is resolved from its **manifest**, not a constant (ADR-0022).
